...
export async function handlerJobDeliveryAttempts(req: Request, res: Response) {
  const { jobId } = req.params;
  ...
  const attempts = await getAllJobDeliveryAttempts(jobId);

  respondWithJSON(res, 200, attempts);
}
